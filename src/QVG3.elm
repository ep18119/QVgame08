...
port receiveMes : (String -> msg) -> Sub msg
...
update msg model = 
    case msg of
        ...
        MsgStartDnD (ASTxy xy (ASTne n b r)) event ->
            if n.getBrickCommand == CommandNOP || model.getDnDInfo.getOnDnD then (model, Cmd.none)
        ...
        GetMessage command ->
            if command == "Reset" then
                ...
                , Cmd.none)
            else (addNewBrick command model, Cmd.none)
        CheckReset ->
            (model, sendMes "Reset")
...
addNewBrick command model =
    let
        ...
        node = { getBrickType = checkType brickCommand
               , getBrickCommand = brickCommand
               , getBrickArgument = { disp = (if brickCommand == CommandFuncStart then "n =" else ""), value = "" }
               }
        ...
        newAST = ASTxy (100,100) (ASTne node Nil right)
    in
...
checkCommand command =
    case command of
      ...
      "CommandTurnBack" -> CommandTurnBack
      "CommandFuncStart" -> CommandFuncStart
      "CommandFuncStop" -> CommandFuncStart --ここ注目
      "CommandIfS" -> CommandIfS
      "CommandIfR" -> CommandIfR
      "CommandIfL" -> CommandIfL
      "CommandIfB" -> CommandIfB
...
checkType command =
    case command of
      ...
      CommandMove -> BasicBrick
      CommandTurnRight -> BasicBrick
      CommandTurnLeft -> BasicBrick
      CommandTurnBack -> BasicBrick
      CommandFuncStart -> EntryBrick
      CommandFuncStop -> TailBrick
      CommandIfS -> CaseBrick
      CommandIfR -> CaseBrick
      CommandIfL -> CaseBrick
      CommandIfB -> CaseBrick
...
view model =
    div [ style "position" "relative"
        ...
        , div[][ button [ onClick CheckReset ][ text " 全ブロック削除 "] ]
        , model.getASTRoots
          |> List.indexedMap (\index astxy -> (String.fromInt index, viewASTxy model astxy)) 
          |> Keyed.node "div" []
        ]
...
viewCommand command =
    case command of
      CommandNOP -> "動作開始"
      CommandMove -> "前進する"
      CommandTurnRight -> "右に進む"
      CommandTurnLeft -> "左に進む"
      CommandTurnBack -> "後ろに進む"
      --CommandTurnSet -> "向きをn度にする"
      --CommandResetXY -> "座標(0,0)に移動"
      --CommandSetX -> "xをnにする"
